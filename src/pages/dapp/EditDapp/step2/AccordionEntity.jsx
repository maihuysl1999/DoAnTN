...
export default function AccordionEntity(props) {
    ...
    function addAttr() {
        ...
        while (
            checkNameAttrUnique(entity.data.attributes, numberAttrOfEntiy, "attribute_" + indexNameAttr, "#") == false
        ) {
            ...
            <div className="accordion_content" style={{ "--attrlength": entity?.data.attributes.length || 0 }}>
                {entity.data.attributes.map((attr, j) => {
                    return (
                        <AttrOfEntity
                            key={attr.idAttr}
                            attr={attr}
                            isPrimaryKey={attr.idAttr === entity.data.primary_key}
                            entityId={entity.id}
                            entityListAttr={entity.data.attributes}
                        />
                    );
                })}
                <div className="accordion_footer">
                    <label htmlFor={`multichoice_colors_${entity.id}`} className="multichoice_colors">
                        <ColorLens />
                        <input
                            className="multi_colors"
                            style={{ display: "none" }}
                            type="checkbox"
                            id={`multichoice_colors_${entity.id}`}
                        />
                            ...
                            {colors.map((color, i) => {
                                return (
                                    <div
                                        ...
                                        onClick={() => changeColorEntity(color)}
                                    ></div>
                                );
                            })}
                        </div>
                    ...
                    <Button
                        variant="outlined"
                        style={{ margin: "11px", textTransform: "capitalize" }}
                        onClick={addAttr}
                    >
                        Add Attribute
                    </Button>
